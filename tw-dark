...
(function () {
  ...
  const aColor = COLORS.lightGray; // HREFs
  const topbar_font_color = COLORS.lightGray; // HREFs

  const buttons_hover_color = COLORS.darkGray; // btn hovers
  const buttons_color = COLORS.lightGray; //Buttons + some fonts

  const buttons_font_color = COLORS.smokeBlack; //Buttons: font color
  const buttons_hover_font_color = COLORS.lightGray; //Buttons: font hover

  const input_font_color = COLORS.lightGray; // input text font

  const decorative_color = COLORS.lightGray; // borders (candy eye)

  const header_color1 = COLORS.darkGray; //header grad 1
  const header_color2 = darkenColor(COLORS.darkGray, 0); //header grad 2
  const header_font_color = COLORS.lightGray; // header font
...
function darkenColor(hex, percent) {
  ...
  addGlobalStyle(`
    ...
    th { color: ${header_font_color}; background-color: ${background_color}, background: transparent; }
...
.bg-top, .left.register, .right.login, .ally-reasons-to-join .reasons .content, .ally-reasons-to-join .reasons .header, .report_transparent_overlay  { background: ${background_color}; }
  ...
  addGlobalStyle(`
    ...
    .widget-button, .border-frame-gold-red, .scavenge-option .title { background: ${buttons_color}; border: 1px solid ${decorative_color}; color: ${buttons_font_color}; box-shadow: 0px 0px 10px ${decorative_color}; transition: background 0.3s, color 0.3s; }

    .btn:hover, .btn-default:hover, #manager_icon_farm:hover, a.manager_icon:nth-child(2):hover, a.thread_button:nth-child(1):hover,
    .bordered-box, a.thread_button:nth-child(2):hover, .widget-button:hover, .anim, .labeled-box-label, .active-skill-list, .active-skill-list .header:not(.icon) { background: ${buttons_hover_color}; color: ${buttons_hover_font_color}; }

    /* Input Fields - font-size, background, border-radius, border, color, padding, box-shadow */
    input, select { font-size: 8pt; background: #000; border-radius: 20px; border: 2px solid ${decorative_color}; color: ${input_font_color}; text-align: center; padding: 6px; box-shadow: 0px 0px 10px ${decorative_color}; }
    ...
    #topdisplay .menu_column .menu-column-item a, .menu_column .menu-column-item a {width: 100%; background: transparent; background-color : ${background_color}; box-shadow: 0px 0px 2px ${decorative_color}; color: ${topbar_font_color}; font-size: 8pt }
    .bottom { display: none; }
